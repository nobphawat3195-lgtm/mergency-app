...
- **การแสดง QR ไม่ใช่การชำระสำเร็จ** สถานะเป็น `PAID` ได้จาก Stripe webhook `payment_intent.succeeded` ที่ตรวจลายเซ็นผ่าน และยอดเงินเป็นบาทตรงกับที่ต้องจ่ายเท่านั้น
...
- ถ้า `PAYMENT_PROVIDER` ไม่ใช่ `stripe` บน production พร้อมเพย์จะปิด ต้องตั้งค่าผู้ให้บริการรับชำระก่อนเปิดรับงานจริง
