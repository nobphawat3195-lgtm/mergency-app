...
- งานตรวจรถ: ช่างถ่ายรูปด้วยกล้องของเครื่องก่อน แล้วแนบจากคลังรูป (`pickMultiImage`) ไม่เปิดกล้องจากในแอป
  - ภาพหลักฐานเฉพาะมุมสำคัญ 12 ช่อง (บังคับ 10) นิยามที่เดียวใน `backend/src/inspections/photo-slots.ts`
  - ทุกช่องมีภาพตัวอย่างมุมถ่าย ต้นฉบับ `tool/brand/guides/<CODE>.svg` สร้าง PNG ด้วย `tool/brand/render_guides.mjs` (เพิ่มช่องใหม่ต้องวาดภาพตัวอย่างและเพิ่มรหัสใน `photoGuideAsset`)
...
- ระบบชำระเงินดู `docs/PAYMENTS.md` (PAID มาจาก webhook ที่ตรวจลายเซ็น Stripe หรือช่างยืนยันเงินสดเท่านั้น)
- ก่อนส่ง store ดู `docs/APP_REVIEW.md` (บัญชีทดสอบ, ลิงก์นโยบาย, ลบบัญชี, App Privacy)
